...
let transl_filter_to_prods (type g) (g : g grammar) indices position {Syntax. lhs; rhs} =
  ...
  |> List.filter_map (fun prod ->
      match matching_dots prod with
      | None -> None
      | Some dots -> Some (prod, dots)
    )

let transl_filter_to_states (type g) (g : g grammar) indices position filter =
  List.fold_right
    (fun (prod, dots) acc ->
       IntSet.fold (fun dot acc ->
           IndexSet.union
             indices.Indices.by_items.:(Item.make g prod dot) acc)
         dots acc)
    (transl_filter_to_prods g indices position filter) IndexSet.empty
